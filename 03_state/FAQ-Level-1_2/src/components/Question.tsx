...
export default function Question() {
  ...
  const [isLongAnswerShown, setIsLongAnswerShown] = useState(false);

  const showAnswerClick = () => {
    setIsAnswerShown(!isAnswerShown);
    if (isLongAnswerShown) {
      setIsLongAnswerShown(!isLongAnswerShown);
    }
  };

  const showLongAnswerClick = () => {
    setIsLongAnswerShown(!isLongAnswerShown);
  };
  ...
  return (
    <div className="faq-block">
      <div className="question">
        ...
        <button type="button" onClick={showAnswerClick}>
          -
        </button>
      </div>
      <div
        className="answer"
        style={{ display: isAnswerShown ? "flex" : "none" }}
      >
        <p>Fast Learning Curve</p>
        <button type="button" onClick={showLongAnswerClick}>
          -
        </button>
      </div>
      <div
        className="longAnswer"
        style={{ display: isLongAnswerShown ? "block" : "none" }}
      >
        <p>
